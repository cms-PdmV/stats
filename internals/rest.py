...
class GetOne(RestIndex):
    ...
    def GET(self, *args):
        ...
        f = urllib2.urlopen('http://localhost:5984/stats/%s' % (args[0]))
        data = f.read()
        return data
...
class ProducesDN(RestIndex):
    ...
    def GET(self, *args):
        # data = [args[0],args[1]]
        __dname = "/".join(args)
        f = urllib2.urlopen('http://localhost:5985/local/stats/_design/lucene/search?q=DN:/%s&include_docs=true' % (__dname))
        data = f.read()
        return data
